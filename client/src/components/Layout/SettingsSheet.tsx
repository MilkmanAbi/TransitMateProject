...
export function SettingsSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Sheet open={open} onClose={onClose} title="You & your commute">
      <p className="mb-2 text-[13px] text-slate-400">Who is TransitMate planning for? Each persona re-weights the same live data differently.</p>
        ...
        {PERSONAS.map((p) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[12px] text-slate-400">{p.blurb}</p>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        <label className="block">
          <span className="text-[12px] font-medium text-slate-400">Usually leave at</span>
        ...
        <label className="block">
          <span className="text-[12px] font-medium text-slate-400">Must arrive by</span>
      ...
      <p className="mt-5 text-[12px] font-medium text-slate-400">Interrupt me when my trip is longer by at least</p>
      ...
      <button onClick={() => set({ largeText: !largeText })} className="mt-5 flex min-h-[52px] w-full items-center justify-between rounded-xl bg-surface-card px-4">
        <span className="text-[15px]">Large text</span>
      ...
      <p className="mt-5 flex gap-2 text-[12px] leading-relaxed text-slate-500">
