    ...
    <article className={`relative animate-rise overflow-hidden rounded-2xl bg-surface-card/80 ring-1 ${s.ring}`}>
      ...
      <div className="py-3 pl-5 pr-4">
        <div className="flex items-start gap-2.5">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-[14px] font-semibold leading-snug text-white">
              {r.simulated && <span className="mr-1.5 rounded bg-amber-500 px-1 py-px align-middle text-[9px] font-black tracking-wider text-black">SIM</span>}
              {r.title}
            </h3>
            <p className="mt-1 text-[13px] leading-relaxed text-slate-300">{r.body}</p>
          </div>
        </div>
        <div className="mt-2 flex items-center justify-between gap-2 pl-7">
          ...
          {r.action && (
            <Link to={r.action.route} className="-my-2 rounded-full px-3 py-2 text-[13px] font-semibold text-brand-400 active:bg-white/5">
              {r.action.label} →
            </Link>
          )}
        </div>
    ...
    <div className="space-y-2.5">
      ...
      {quiet.length > 0 && (
        <div className="rounded-2xl border border-dashed border-white/10">
          <button onClick={() => setOpen(!open)} className="flex min-h-[48px] w-full items-center gap-2 px-4 text-left text-[13px] text-slate-400">
